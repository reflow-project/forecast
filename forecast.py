...
def main():
    ...
    for resource in sold['resource_name'].unique():
        ...
        if df_test(resource, sold_resource):
            ...
            print(f"SES RMS Error with auto optimization (smoothing_level = {ses_fit.model.params['smoothing_level']}) is {round(np.sqrt(mse2), 2)}")
            ...
            sarima_fit = sarima(y_to_train,3)
            ...
            sarima_all = pd.Series(index=arima_all.index, data=np.concatenate((sarima_fit.predict(),sarima_fcast)))
            ax1.plot(sarima_all, ".-", color='y', label = 'sarima')
            


        else:
            model = auto_arima(sold_resource.sold,
                        start_p=0,
                        start_q=0,
                        max_p=5,
                        max_q=5,
